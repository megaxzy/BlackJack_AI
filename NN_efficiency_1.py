...
def judge_chose(result,can_drop):
    ...
    chose_action = ""
    if can_drop==0:
        if (result[0] <=0.25): chose_action = actions[2]
        elif (result[0] >= 0.8 ): chose_action = actions[1]
        else: chose_action = actions[0]
    else:
        if (result[0] >= 0.7 ): chose_action = actions[1]
        else: chose_action = actions[0]
    return chose_action
...
class efficiency_class():
    ...
    def calculate(self):
        ...
        for i in range(0,5):
            ...
            while(1):
                test_item=np.zeros((1,39))
                for x in range(0,self.game.cards.matrix.shape[0]):
                    for y in range(0,self.game.cards.matrix.shape[1]):
                        if self.game.cards.matrix[x][y][0] == 1: test_item[0][0+ y % 13]+=1
                        if self.game.cards.matrix[x][y][1] == 1: test_item[0][13+y % 13]+=1
                        if self.game.cards.matrix[x][y][2] == 1: test_item[0][26+y % 13]+=1
